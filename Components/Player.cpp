...
void CPlayerComponent::UpdateCamera(float frameTime)
{
	...
	if (ICharacterInstance *pCharacter = m_pAnimationComponent->GetCharacter())
	{
		...
		m_cameraOrientation = pCharacter->GetISkeletonPose()->GetAbsJointByID(m_cameraJointId);
		// Apply the offset to the camera
		localTransform.SetTranslation(m_cameraOrientation.t + Vec3(0, viewOffsetForward, viewOffsetUp));
	}
...
void CPlayerComponent::UpdateLookat(float frameTime)
{
	/*ray_hit rayHit[10];
	Vec3 startPos = m_pCameraComponent->GetCamera().GetPosition();
	Vec3 direction = m_pCameraComponent->GetCamera().GetViewdir().GetNormalized() * 10;

	int hitCount = gEnv->pPhysicalWorld->RayWorldIntersection(startPos, direction,
		ent_rigid | ent_sleeping_rigid | ent_independent | ent_static | ent_areas, rwi_colltype_any | rwi_ignore_noncolliding | rwi_stop_at_pierceable,
		&rayHit[0], 2, GetEntity()->GetPhysicalEntity());

	gEnv->pAuxGeomRenderer->DrawLine(GetEntity()->GetPos(), startPos - direction, { 0,255,0 }, { 0,255,0 });*/

	//bool bOpen = true;
	/*ImGui::Begin("Hits", &bOpen, ImGuiWindowFlags_AlwaysAutoResize);
	ImGui::Text("Num Hits: %i", hitCount);
	ImGui::Separator();
	for (int i = 0; i < hitCount; ++i)
	{
		auto& hit = rayHit[i];
		auto pEnt = gEnv->pEntitySystem->GetEntityFromPhysics(hit.pCollider);
		if (pEnt)
		{
			ImGui::Text(pEnt->GetName());

			AABB bBox;
			pEnt->GetLocalBounds(bBox);
			auto transform = pEnt->GetWorldTM();
			gEnv->pAuxGeomRenderer->DrawAABB(bBox, transform, false, { 255,0,0 }, EBoundingBoxDrawStyle::eBBD_Faceted);
		}
		else
		{
			ImGui::Text("Pt: {%.2f|%.2f|%.2f}", hit.pt.x, hit.pt.y, hit.pt.z);
			gEnv->pAuxGeomRenderer->DrawSphere(hit.pt, 0.5, { 255,0,0 });
		}
